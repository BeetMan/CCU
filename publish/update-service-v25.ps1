# CCUService v25 安全部署脚本
# 需要管理员权限。执行时会备份当前服务路径；v25 启动失败则自动回滚。
...
$targetExe = Join-Path $PSScriptRoot 'CCU.Service.v25\CCU.Service.exe'
$backupDir = Join-Path $env:ProgramData 'CCU_Alternative'
$backupFile = Join-Path $backupDir 'deployment-backup-v25.json'
...
if (-not (Test-Path $targetExe)) { throw "未找到 v25 服务文件: $targetExe" }
...
try {
    ...
    Write-Host '[2/3] 切换到 v25...' -ForegroundColor Yellow
    Set-ServiceBinaryPath $targetExe

    Write-Host '[3/3] 启动 v25...' -ForegroundColor Yellow
    ...
    Write-Host ''
    Write-Host '✓ CCUService v25 已启动。下一步只运行只读冒烟测试：' -ForegroundColor Green
    Write-Host '  powershell -ExecutionPolicy Bypass -File tests\smoke.ps1 -Version v25' -ForegroundColor Cyan
}
catch {
    Write-Host "✗ v25 部署失败: $($_.Exception.Message)" -ForegroundColor Red
