...
- [ ] D-1 架构路线：EC 直写优先 + MQTT 兜底（推荐），还是全 MQTT 先行？
- [ ] 阶段 0 需要重启 CCUService 数次 + 一次整机重启（验证 B4），何时方便？
- [ ] 阶段 2.1 风扇曲线写入涉及 SMAPCTABLE（比单寄存器风险高），是否接受"先备份再试写"的验证流程？
- [ ] D 盘仓库是否推 GitHub 私有远程（BeetMan 名下）？
